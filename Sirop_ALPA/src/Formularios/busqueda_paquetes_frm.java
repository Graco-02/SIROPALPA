...
public class busqueda_paquetes_frm extends JPanel{
	...
	public busqueda_paquetes_frm(ResultSetTableModel resultado){
	...
	private JTextField get_txtFecha_Ini(){
		...
		txt_fecha_ini.setFont(new Font(Font.DIALOG_INPUT,Font.BOLD,20));
	...
	private JTextField get_txtFecha_Fin(){
		...
		txt_fecha_fin.setFont(new Font(Font.DIALOG_INPUT,Font.BOLD,20));
	...
	private void set_query(){
		try {
			...
			resultado.setQuery(new ManejadorReportes().get_query_paquetes(fecha_ini.toString(), fecha_fin.toString()));
